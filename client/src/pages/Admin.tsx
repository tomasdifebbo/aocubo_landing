...
export default function Admin() {
    ...
    const [activeTab, setActiveTab] = useState<"analytics" | "corretor" | "feed">("corretor");
    const [page, setPage] = useState(1);
    const [searchQuery, setSearchQuery] = useState("");
    const [statusFilter, setStatusFilter] = useState("all");
    const [sortByM2, setSortByM2] = useState<boolean>(false);
    ...
    // Optional sort by Price per m²
    if (sortByM2) {
        filteredProperties = [...filteredProperties].sort((a, b) => {
            const m2A = a.area > 0 ? a.price / a.area : 0;
            const m2B = b.area > 0 ? b.price / b.area : 0;
            return m2A - m2B;
        });
    }
    ...
    const handleSendWhatsAppPitch = (p: any) => {
        const pricePerM2 = p.area > 0 ? Math.round(p.price / p.area) : 0;
        const estDown = Math.round(p.price * 0.2);
        const text = `🏢 *${p.title}* - ${p.neighborhood}%0A%0A💰 *Preço:* R$ ${p.priceFormatted}%0A📐 *Metragem:* ${p.area}m² (R$ ${pricePerM2.toLocaleString("pt-BR")}/m²)%0A🛏️ *Dormitórios:* ${p.bedrooms === 0 ? "Studio" : `${p.bedrooms} dorms`}%0A🚗 *Vagas:* ${p.parkingSlots}%0A🏁 *Estágio:* ${p.status}%0A%0A💡 *Entrada Estimada (20%):* R$ ${estDown.toLocaleString("pt-BR")}%0A%0A📲 *Veja fotos e ficha técnica:*%0A${p.url}`;
    ...
    return (
        <div className="min-h-screen flex flex-col bg-slate-950 text-slate-100 font-sans selection:bg-amber-500/30 selection:text-amber-200">
            ...
            <main className="flex-1 pb-24">
                {/* Hero Header */}
                <section className="relative pt-10 pb-10 md:pt-14 md:pb-14 border-b border-slate-800/80 bg-gradient-to-b from-slate-900 via-slate-950 to-slate-950 overflow-hidden">
                    ...
                    <div className="container relative z-10">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
                            <div>
                                <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-400 text-xs font-bold uppercase tracking-widest mb-3">
                                    ...
                                    Suite Master do Corretor & Vendedor
                                </div>
                                <h1 className="text-3xl md:text-5xl font-serif italic text-white tracking-tight">
                                    Central de Vendas & Gestão
                                </h1>
                                <p className="text-sm md:text-base text-slate-400 font-light mt-2 max-w-2xl">
                                    Simulação de financiamento, comparador de R$/m², gerador de fichas para WhatsApp e catálogo atualizado.
                                </p>
                            </div>
                        ...
                        {/* Navigation Tabs */}
                        <div className="flex flex-wrap items-center gap-2 md:gap-3 border-b border-slate-800/80 pb-4">
                            <button
                            ...
                            >
                                ...
                                🏢 Catálogo AoCubo ({properties.length})
                            </button>
                        </div>
                    </div>
                </section>
                ...
                <div className="container py-8">
                    {/* TAB 1: Central do Corretor */}
                    {activeTab === "corretor" && (
                        <div className="space-y-8">
                            <FinancingCalculator />
                            <PropertyComparator availableProperties={properties} />
                        </div>
                    )}
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                <Card className="bg-slate-900/60 border-slate-800/80 backdrop-blur-md text-white rounded-2xl p-5 shadow-lg">
                                    ...
                                    <div className="flex items-center gap-1 mt-1 text-xs text-emerald-400 font-medium">
                                        ...
                                        <span>Visitas reais monitoradas</span>
                                    </div>
                                </Card>
                                ...
                                <Card className="bg-slate-900/60 border-slate-800/80 backdrop-blur-md text-white rounded-2xl p-5 shadow-lg">
                                    ...
                                    <div className="flex items-center gap-1 mt-1 text-xs text-blue-400 font-medium">
                                        ...
                                        <span>Contas ativas no Supabase</span>
                                    </div>
                                </Card>
                        ...
                        <div className="space-y-8">
                            ...
                            <div className="bg-slate-900/80 border border-slate-800/80 rounded-2xl p-4 md:p-6 backdrop-blur-md flex flex-col md:flex-row gap-4 justify-between items-center">
                                <div className="relative w-full md:w-96">
                                    ...
                                    <Input
                                        placeholder="Buscar por título, bairro, construtora ou ID..."
                                        value={searchQuery}
                                ...
                                <div className="flex flex-wrap items-center gap-3 w-full md:w-auto justify-end">
                                    <Button
                                        type="button"
                                        onClick={() => setSortByM2(!sortByM2)}
                                        variant="outline"
                                        className={`h-11 rounded-xl text-xs gap-1.5 border ${
                                            sortByM2
                                    ...
                                    >
                                        ...
                                        {sortByM2 ? "Menor R$/m² Ativado" : "Ordenar p/ R$/m²"}
                                    </Button>
                                    ...
                                    <div className="flex items-center gap-2 w-full md:w-auto">
                                        ...
                                        <select
                                            ...
                                            className="h-11 w-full md:w-52 bg-slate-950 border border-slate-800 text-white rounded-xl px-3 text-xs"
                                        >
                                            <option value="all">Todos os Estágios</option>
                                            <option value="Pronto">Pronto para Morar</option>
                                            <option value="Em obras">Em Construção</option>
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="relative h-56 bg-slate-950 overflow-hidden">
                                                    ...
                                                    {/* Price per m2 Badge */}
                                                    {pricePerM2 > 0 && (
                                                        <div className="absolute top-3 left-3 flex items-center gap-1 px-3 py-1 rounded-full bg-slate-950/90 backdrop-blur-md text-amber-300 text-[10px] font-bold border border-amber-500/30">
                                                ...
                                                <div className="p-6 flex-1 flex flex-col justify-between space-y-4">
                                                    ...
                                                    <div className="space-y-3 pt-2">
                                                        ...
                                                        {/* WhatsApp Quick Pitch Button */}
                                                        <Button
                                                            onClick={() => handleSendWhatsAppPitch(p)}
                                                            className="w-full bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-xl h-10 text-xs gap-1.5 shadow-md"
                                                        >
                                                            <Send className="w-3.5 h-3.5" />
                                                            📱 Gerar Proposta p/ WhatsApp
                                                        </Button>
                                                        ...
                                                        <div className="grid grid-cols-2 gap-2">
                                                            <Link href={`/imovel/${p.slug}/${p.id}`}>
                                                                <Button
                                                                    variant="outline"
                                                                    className="w-full text-xs border-slate-700 bg-slate-800/50 hover:bg-slate-800 text-white rounded-xl h-9"
                                                                >
                                                                    Ver no Site
                                                                </Button>
                                                            </Link>
                                                            <Button
                                                                ...
                                                                className="w-full text-xs bg-amber-500 hover:bg-amber-400 text-slate-950 font-bold rounded-xl h-9 gap-1"
                                                            >
